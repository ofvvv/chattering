...
function scheduleDockAutoScroll() {
    try {
        ...
        window.electronAPI.logError(`[scheduleDockAutoScroll] ${e.message}`);
    }
}
...
if(dockScroll){
    try {
        ...
        window.electronAPI.logError(`[dockScroll-listeners] ${e.message}`);
    }
}
...
function addToDock(type,user,actionText,giftImgUrl,noAnim=false){
    try {
        ...
        window.electronAPI.logError(`[addToDock] ${e.message}`);
    }
}
function updateLikeRow(user,uid,count){
    try {
        ...
        if(existingLike) {
            ...
            body.appendChild(uSpan);body.appendChild(aSpan)
        ...
        window.electronAPI.logError(`[updateLikeRow] ${e.message}`);
    }
}
