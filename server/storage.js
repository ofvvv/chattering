// server/storage.js — Chattering v3.1
'use strict'
const fs   = require('fs')
...
function procesarUsuario(userId, username, platform) {
...
function getState(key)        { return dbGet('SELECT value FROM state WHERE key=?', [key])?.value ?? null }
function setState(key, value) { dbRun('INSERT OR REPLACE INTO state (key,value) VALUES (?,?)', [key, String(value)]); scheduleSave() }
...
function appendLog(obj) { try { fs.appendFileSync(getLogPath(), JSON.stringify(obj)+'\n') } catch {} }
...
function getHistory()     { return readLastLines(getLogPath(), 200).filter(l=>l.type==='msg').slice(-50) }
function getDockHistory() { return readLastLines(getLogPath(), 500).filter(l=>(l.type==='follow'||l.type==='gift')&&l.sessionStart===SESSION_START) }
...
function getTotalLikes() { return granTotalLikes }
function addLikes(n) { granTotalLikes += n; setState('totalLikes', granTotalLikes); return granTotalLikes }

module.exports = { initDB, procesarUsuario, getState, setState, appendLog, getHistory, getDockHistory, getUserHistory, SESSION_START, getTotalLikes, addLikes }
